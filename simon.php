...
<section class="container">
	<div class="row-fluid">
		<article class="span7">
			...
			function kayleeNotDefault($path){
				//uhh this is dumb but reaplace every space with a space...
				$handle = fopen($path,"r");
				$content = fread($handle,filesize($path));
				$content = str_replace("\$kaylee = false;", "\$kaylee = true;", $content);
				//oppen again to write
				$handle = fopen($path,"w");
				fwrite($handle,$content);
				fclose($handle);
			}
			kayleeNotDefault("./kaylee.php");

			
			?>
		</article>
		<aside class="span5">
			<a href="processors/wiper.php" data-ajax="wiper" class="btn-grey btn-small">wipe serenity</a>
			...
			<br>
			<a href="/index.php" class="btn-main btn-small">to index</a>
		</aside>
	</div>
